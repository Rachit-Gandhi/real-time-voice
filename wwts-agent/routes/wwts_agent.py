...
from wwts_agent.hinglish import to_hinglish
...
def wwts_invoke(request: WWTSInvokeRequest):
    ...
    if context.get("hinglish"):
        answer = to_hinglish(answer)
        # Re-translate independently only when speak differs from answer, else
        # reuse so the two stay consistent and we avoid a second model call.
        speak = answer if speak == state.get("final_answer", "") else to_hinglish(speak)
    return {
        "answer": answer,
        "speak": speak,
