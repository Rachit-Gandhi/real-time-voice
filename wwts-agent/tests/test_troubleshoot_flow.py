...
def test_product_ref_extraction_skips_filler_words():
    ...
    assert C._extract_product_ref("The product reference for this particular device is BDQ.") == "BDQ"
    assert C._extract_product_ref("product reference BDQ") == "BDQ"
    assert C._extract_product_ref("BDQ") == "BDQ"


def test_failed_overheating_troubleshoots_keep_original_product_ref():
    s = {
    ...
    s = step(s, "Hey, my name is Rahul")
    assert s["stage"] == "ts_confirm_issue"

    s = step(
        s,
        "My Dell Latitude Laptop is overheating and freezing a lot, "
        "the product reference for this particular device is BDQ.",
    )
    assert s["stage"] == "ts_troubleshooting"
    assert s.get("ts_product_ref") == "BDQ"

    s = step(s, "No, I have already tried this troubleshoot and it did not help me out.")
    assert s["stage"] == "ts_troubleshooting"
    assert s.get("ts_product_ref") == "BDQ"

    s = step(s, "No, the problem is still there and this did not resolve my issue.")
    assert s["stage"] == "collecting_create"
    assert (s.get("create_fields") or {}).get("Product Reference") == "BDQ"

    s = step(
        s,
        "My customer code is DELLQXS and my contact phone is 9650470567, "
        "the street address is 1 Delway, the city is Round Rock, "
        "the state is Texas and the zip code is 78682.",
    )
    assert s["stage"] == "confirming_create"
    assert (s.get("create_fields") or {}).get("Product Reference") == "BDQ"
    assert "product BDQ" in s["final_answer"]
