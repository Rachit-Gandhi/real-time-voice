...
def _missing_create_fields(fields: dict, has_customer_code: bool, multi_codes: bool = False) -> list[str]:
    ...
    if not has_site and not has_location:
        # Word the ask for the full street address so callers tend to give it up
        # front (the validator needs it to geocode), while city/state/ZIP alone is
        # still accepted — a failed validation then escalates to the street.
        missing.append("Street Address, City, State, and ZIP")
    return missing
...
CONFIRMING_CREATE
  ...
  ════════════════════════════════════════════
  EXTRACTION DISCIPLINE FOR create_fields (applies to EVERY field)
  ════════════════════════════════════════════
  Each field corresponds to ONE specific thing. Read the sentence semantically and
  only fill a field when the caller actually states THAT field's value:
    • A field already set in "Create WO fields so far" must be KEPT as-is. Only
      change it when the caller explicitly gives a new value for THAT field
      (e.g. "the product is BDQ", "change the contact to Priya", "BDQ not BBQ").
    • NEVER pull a value for one field out of another field's content — e.g. do
      NOT take a Product Reference out of an address/street/ZIP/phone/name, do NOT
      take a Contact Name from "phone number for X", do NOT take a city from a
      product code. Cross-field bleed is the #1 mistake — avoid it.
    • NEVER take a filler/sentiment word as a field value. From "you got my product
      wrong, my product is BDQ", the product is BDQ — it is NOT "wrong". From "no,
      that's incorrect", nothing changes — "incorrect"/"wrong"/"no" are not values.
    • Product Reference is a short alphanumeric CODE (e.g. BDQ, AB12); never a word
      like for/the/device/wrong, and never invented.
    • When you are not sure a field was explicitly given, return null for it so the
      known value is preserved. Returning null is always safer than guessing.
...
_NAME_PATTERNS = (
    ...
    # Romanised Hinglish: "mera naam Rachit hai", "main Rachit hoon", "naam Rachit".
    # Single-token capture (no spaces) so trailing "hai/hoon" isn't swallowed.
    r"\bmera naam\s+([A-Za-z][A-Za-z'.-]{1,30})",
    r"\bmain\s+([A-Za-z][A-Za-z'.-]{1,30})\s+h(?:oon|u|un|ai|on)\b",
    r"\bnaam\s+([A-Za-z][A-Za-z'.-]{1,30})",
)


# Filler / greeting / affirmation words that are NOT names. A bare reply made up
# entirely of these ("Thanks.", "Hello", "Yes okay") must not be taken as a name.
_NON_NAME_WORDS = {
    "thanks", "thank", "thankyou", "you", "ok", "okay", "yes", "yeah", "yep",
    "no", "nope", "sure", "please", "nothing", "none", "hi", "hey", "hello",
    "howdy", "hiya", "greetings", "sup", "yo", "bye", "goodbye", "cheers",
    "good", "morning", "afternoon", "evening", "night", "fine", "great", "cool",
    "alright", "right", "well", "um", "uh", "hmm", "maybe", "idk", "dunno",
    "what", "who", "why", "how", "huh", "pardon", "sorry", "the", "my", "name",
    "is", "it", "im", "i", "am", "and", "a", "an",
}


def _is_non_name(word: str) -> bool:
    return word.lower().strip(".'-!,") in _NON_NAME_WORDS
...
def _extract_name(text: str) -> str | None:
    ...
    # Reject replies that are purely filler/greeting words ("Thanks.", "Hello").
    if not _mentions_issue(t):
        words = t.rstrip(".!,").split()
        if (
            1 <= len(words) <= 3
            and all(re.fullmatch(r"[A-Za-z.'\-]+", w) for w in words)
            and any(not _is_non_name(w) for w in words)
        ):
            return _clean_name(t.rstrip(".!,"))
    return None


def _recent_user_texts(messages: list, n: int = 5) -> list[str]:
    """The last `n` user-turn texts, newest first — for re-scanning recent turns
    when voice VAD split one utterance across several turns."""
    texts = [
        str(m.get("content") or "")
        for m in (messages or [])
        if m.get("role") == "user" and str(m.get("content") or "").strip()
    ]
    return list(reversed(texts[-n:]))


def _name_said_by_caller(name: str, messages: list, user_message: str) -> bool:
    """True if any token of `name` actually appears in what the caller said —
    guards against the LLM extractor inventing a name the caller never gave.

    Exception: when the caller wrote in a non-Latin script (e.g. Devanagari
    Hindi), the extractor transliterates to Roman, so the romanised name won't
    appear verbatim — trust it there rather than wrongly rejecting it."""
    hay = " ".join(
        [user_message or ""]
        + [m.get("content", "") for m in (messages or []) if m.get("role") == "user"]
    )
    if re.search(r"[^\x00-\x7f]", hay):  # any non-ASCII (Hindi/Devanagari) → trust LLM
        return True
    return any(tok.lower() in hay.lower() for tok in re.findall(r"[A-Za-z]{2,}", name or ""))
...
_PRODUCT_STOPWORDS = {
    ...
    # Words that show up around corrections but are never product codes.
    "wrong", "incorrect", "mistaken", "right", "correct", "not", "got", "you",
    "site", "address", "street", "city", "state", "zip", "postal",
    # Filler words between "product reference" and the actual code.
    "particular",
}
...
def _extract_product_ref_correction(text: str) -> str | None:
    ...
    # "product (reference) is/=/: X" — REQUIRE the connector (a bare "product X"
    # would grab "wrong" from "you got my product wrong"). Scan all matches so
    # "...my product wrong, my product is BDQ" still resolves to BDQ.
    for m in re.finditer(
        r"\bproduct(?:\s+reference)?\s*(?:is|=|:)\s*([A-Za-z][A-Za-z0-9\-]{1,14})\b",
        t, re.IGNORECASE,
    ):
        if m.group(1).lower() not in _PRODUCT_STOPWORDS:
            return m.group(1).upper()
...
def _handle_troubleshoot(
    ...
    *,
    ...
    # Only trust an LLM-extracted name the caller actually said — the extractor
    # sometimes invents a placeholder ("Alex Rivera") for a non-name reply.
    llm_name = _coerce_none(ext.get("name"))
    if llm_name and not _name_said_by_caller(llm_name, messages, user_message):
        llm_name = None
    cand_name = llm_name or _extract_name(user_message)
    ...
    # History fallback: voice VAD can split one breath into separate turns, so the
    # product reference may sit in a slightly earlier user turn. Scan the recent
    # user messages (with an explicit product cue) before giving up and re-asking.
    if not cand_ref and not product_ref:
        for txt in _recent_user_texts(messages, n=5):
            if re.search(r"(?i)\b(product|reference|model|ref|sku)\b", txt):
                cand_ref = _extract_product_ref(txt)
                if cand_ref:
                    break
    if cand_ref:
    ...
    # History fallback for the symptom too, for the same split-utterance reason.
    if not issue:
        for txt in _recent_user_texts(messages, n=5):
            if _is_concrete_issue(txt):
                issue = txt.strip()
                break
...
def _create_llm_extract(messages: list, known: dict, customer_codes: list) -> dict:
    ...
    try:
        ...
        sys = (
            ...
            "EXTRACTION DISCIPLINE (every field): only fill a field when the caller "
            "explicitly states THAT field's value. Keep already-known values unless the "
            "caller explicitly gives a new one for that exact field. NEVER pull one field's "
            "value out of another's content (no Product Reference from an address/phone/name, "
            "no Contact Name from 'phone for X', etc.). NEVER take a filler/sentiment word as a "
            "value — in 'you got my product wrong, my product is BDQ' the product is BDQ, not "
            "'wrong'. Product Reference is a short alphanumeric CODE, never a word, never "
            "invented. When unsure, return null so the known value is preserved.\n"
...
def _apply_create_corrections(
    ...
    elif create_fields.get("Product Reference"):
        # Product is already known — don't let a loosely-extracted token from an
        # unrelated reply (address, customer code, etc.) clobber it. Only an
        # explicit correction above may change a known Product Reference.
        updates.pop("Product Reference", None)
...
def converse(state: WWTSState) -> WWTSState:
    ...
    # An explicit product correction ("my product is BDQ", "change to BDQ", "BDQ
    # not BBQ"). Used to allow a deliberate change while still blocking the LLM
    # from clobbering a known Product Reference with one it hallucinates from an
    # address/contact reply (that's how "BDQ" became "DID").
    product_correction = _extract_product_ref_correction(user_message) if user_message else None
    ...
    for key, val in raw_create.items():
        ...
        if (
            ...
            and create_fields.get(key)
            and coerced.upper() != str(create_fields[key]).upper()
        ):
            # Known product differs from the LLM's value — only change it on an
            # explicit correction, and use the corrected value (not the LLM's,
            # which may be a stray word like "WRONG").
            continue
        create_fields[key] = coerced
    if product_correction:
        create_fields["Product Reference"] = product_correction
...
def _extract_create_fields_from_text(text: str, customer_codes: list) -> dict:
    ...
    if loc:
        ...
        city_state_zip = re.search(
            r"\bcity\s*(?:is|=|:)?\s*([A-Za-z][A-Za-z .'-]{1,40}?)\s*,?\s+"
            r"(?:the\s+)?state\s*(?:is|=|:)?\s*([A-Za-z][A-Za-z .'-]{1,30}?)\s+"
            r"(?:and\s+)?(?:the\s+)?(?:zip|postal)(?:\s+code)?\s*(?:is|=|:)?\s*(\d{5}(?:-\d{4})?)\b",
            text,
            re.IGNORECASE,
        )
        if city_state_zip:
            found["Customer City"] = city_state_zip.group(1).strip().rstrip(".,")
            found["Customer State"] = city_state_zip.group(2).strip().rstrip(".,")
            found["Customer Postal Code"] = city_state_zip.group(3).strip()
...
def _fallback(
    ...
    if _is_pure_greeting(user_message):
        ...
        multi_codes = len(customer_codes) > 1
        if user_message:
            extracted = _extract_create_fields_from_text(user_message, customer_codes)
            # Don't let loose extraction clobber an already-known Product
            # Reference (e.g. grabbing "DID"/"ROUND" out of an address reply);
            # only an explicit correction may change it.
            if (
                create_fields.get("Product Reference")
                and "Product Reference" in extracted
                and not _extract_product_ref_correction(user_message)
            ):
                extracted.pop("Product Reference", None)
            create_fields.update(extracted)
            create_fields = _normalize_create_fields(create_fields)
        if user_message:
