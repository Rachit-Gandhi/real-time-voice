    ...
    async def create_client_secret(
        ...
        *,
        ...
        if tool_defs:
            ...
            # Always "auto": the model must be free to SPEAK the tool result after
            # the frontend feeds it back. "required" forces another tool call every
            # turn, which loops run_wwts on the same utterance and makes the agent
            # march through the script with no user input. Strong instructions
            # (registry.py) already make the model call run_wwts first.
