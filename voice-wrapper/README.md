...
| `TRANSCRIPT_DB_PATH` | `./data/transcripts.db` | SQLite file for saved call/chat transcripts |
...
When a voice call or text chat is ended (the **End** button in the console), the
full conversation is POSTed to `/transcripts` and stored in SQLite
(`TRANSCRIPT_DB_PATH`). Browse saved conversations at **`/dashboard`** (linked
from the console header): a list pane of past sessions and a reader pane showing
the turn-by-turn thread. Filter to the signed-in user or toggle **All users**.

Transcript API:

| Method | Path | Purpose |
|---|---|---|
| `POST` | `/transcripts` | Save a finished conversation `{turns, user_id, mode, ...}` |
| `GET` | `/transcripts?user_id=` | List transcript summaries (newest first) |
| `GET` | `/transcripts/{id}` | Full transcript with all turns |
