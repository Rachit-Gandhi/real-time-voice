...
@pytest.mark.asyncio
async def test_create_client_secret_includes_run_wwts_tool(monkeypatch):
    import json
    ...
    # "auto" (not "required"): forcing a tool call every turn loops run_wwts on the
    # same utterance and makes the agent run through the script with no user input.
    assert session["tool_choice"] == "auto"
